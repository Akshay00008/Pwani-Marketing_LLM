...
def create_langraph_workflow(llm, prompt, input_vars, output_format, use_search_engine=False, search_engine_query=None):
    ...
    def generate_content(state):
        try:
            ...
            if rag_context_str:
                prompt.template = prompt.template.replace('{rag_context_str}', rag_context_str)
                logging.info(f"Updated prompt template with RAG context:\n{prompt.template}")
            else:
                logging.info("No RAG context available for prompt enhancement")
