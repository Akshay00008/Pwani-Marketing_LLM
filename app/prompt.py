...
from rag import RAGSystem  # Import RAGSystem - although we might not need it directly here anymore
from llm import get_llm # Import get_llm - for default RAG system initialization
import os

# Initialize RAG system with a default LLM (moved initialization here for prompt context)
default_llm = get_llm(os.getenv('OPENAI_API_KEY'), "gpt-4", temperature=0)
rag_context = RAGSystem(default_llm) # Initialize RAG system here
...
"""
        ...
        return PromptTemplate(
            ...
            input_variables=[
                ...
                "tone_style",
                "search_results",
                "rag_context" # Added rag_context as input variable
            ],
            partial_variables={
